...
dimension: dim X=d=2n>=14; m=2; V(14)=102, V(d)=8d-16 for d=16,18,20,22, and V(d)=7d+6 for even d>=24; h_Z(1)=V(d); delta_1=V(d)-d-1; slack s_10(d)=2(V(d)-d-1); N=2V(d)
codimension: construct the complete G144 package with delta_1=V(d)-d-1 and an isomorphic degree-one relation transport at the valid B266/B271/B272/B273/B276 standard-cubic piecewise boundary
...
claim: For every arbitrary primitive target, construct the complete G144 package at h_Z(1)=V(d), delta_1=V(d)-d-1, slack s_10(d)=2(V(d)-d-1), and N=2V(d), retaining every relation, ODP, Kuranishi, rational-type, and nonzero specified-pairing clause.
falsifier: one primitive target for which no polarization realizes the V(d) package or a different boundary after the valid closure of G195
---

# G196 — Active standard/cubic piecewise boundary

B276 moves the \(d=14\) row directly to rank 102:

\[
\begin{array}{c|c|c|c}
 d & V(d) & s_{10}(d) & \text{polarizations not yet excluded}\\ \hline
 14 & 102 & 174 & A=O_Q(1)\\
 16 & 112 & 190 & A=O_Q(1)\\
 18 & 128 & 218 & A=O_Q(1)\\
 20 & 144 & 246 & A=O_Q(1)\\
...
G196 must classify standard equality at rank 102 on \(Q^{14}\),
