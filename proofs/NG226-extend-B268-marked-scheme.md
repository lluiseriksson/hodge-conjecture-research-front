...
status: NO-GO
base_field: C
variety: the smooth split even-dimensional quadric X=Q^d with d=2n>=22, any seven planar supports in the 3+3 form of B269, cubic A=O_Q(3), and H=O_Q(6)
...
codimension: the failed route is to retain an exact 3+3 cubic equality rank while adjoining the remaining reduced marked supports required by G190
...
claim: No exact seven-support cubic equality witness in 3+3 form can be promoted to a G190 marked scheme without raising h_Z(1), because every eighth distinct double neighborhood contributes a nonzero new restriction coordinate.
...
# NG226 — A \(3+3\) witness cannot be extended to the marked scheme

- **Label:** NO-GO
- **Route:** keep an exact \(3+3\) rank \(7d+5\) while adjoining the
...
- **Scope guard:** this applies to every \(3+3\) configuration but needs
  a separate theorem that cubic equality forces that form.
