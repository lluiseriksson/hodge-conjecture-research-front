...
- **Infrastructure:** 88% - independent Git repository, mandatory topology,
  ...
  seed exist.
- **Formal proof-chain closure:** 98% - B007 provides a fully quantified
  ...
  \(|\Delta_m|/(mn-c)\to\infty\) for \(A_m=H^m\). B142 realizes this scale
  in every middle dimension on an anchored product fiber, with \(m^n\)
...
- B010 - PROVED: a singular member detects \(\zeta\) exactly when one of its
  type-\((0,0)\) relation classes \(\gamma_\beta\) pairs nontrivially with
  \(\zeta\).
...
- B035 - PROVED: the smallest simple central smoothing arrangement needing
...
- B046/G018 - PROVED: one codimension-three flat nested in a
...
- B048/G020 - PROVED: in the explicit fork, the common-parent blow-up
  separates the child planes; both child orders give the same full
  type-\((0,0)\) relation kernel and no degree-one lower support.
- B049/G021 - PROVED: every representable wonderful building set has the
  intrinsic Picard basis and multiplicity-one strict-branch formula in every
...
- B138 - PROVED: a minimal dependent adjoint circuit is
  \(\mathrm{CB}(mn-c)\); S057 and the carrier first-jet calculation force
...
- B140 - PROVED: S059 and a uniform bounded-component lemma force one
...
- B142 - PROVED: on \(\mathbf P^n\times\mathbf P^n\), a general
  fiber-containing \((m,m)\) divisor realizes \(m^n\) isolated nodes, a
  uniform smoothing matroid partitionable into \(n!\) independent blocks,
  adjoint defect one, and a rank-one primitive pairing. The witness is
  algebraically anchored and supplies no arbitrary-class construction.
- B143 - PROVED: the B142 labeled discriminant branches form a Li clean
...
- B145 - PROVED: at an ordered nodal pair, Hessian inversion identifies the
  incidence tangent with the kernel of value evaluation; rank-smooth excess
  projects to G090's saturated germ.
- G091 - EXPLORATORY: construct a carrier-free smooth excess ordered-node
  component with \(R<N\), positive adjoint defect, and nonzero specified
  pairing.
...
- NG119 - NO-GO: a generic surjective conditional-gradient map stays
  ...
  \(nN>n\).
- B149 - PROVED: G093's projected-gradient rank \(\rho\) is equivalent to
  an oriented half-double scheme of length \((n+1)N\) with coherent defect
  \((n+1)N-R-\rho\).
- G094 - EXPLORATORY: construct that scheme with evaluation rank at most
  \(R+n\), Hessian compatibility, smooth integration, and nonzero specified
  pairing.
...
- B201 - PROVED: after quadratic closure, the cubic tensor has only the
  pure block \(\Theta\) on \(U^3\) and the mixed block
  \(\Xi:\overline K\to(\mathcal T/S)\otimes\operatorname{Sym}^2U^*\);
  all blocks with at least two double directions vanish.
- G131 - EXPLORATORY: kill both \(\Theta\) and \(\Xi\) in the full
...
- NG197 - NO-GO: one dimension beyond three tangent osculators does not
  realize G148.
- B240 - PROVED: sextic and quartic first-jet separation plus standard
  contact rigidity exclude two dimensions beyond three doubles; hence
  \(m=2\) requires \(s\ge4d+10\).
- G163 - NO-GO: the layers \(s=4d+8\) and \(s=4d+9\) fail on every valid
  even quadric.
...
- G193 - NO-GO: B274 excludes its sole standard rank-95 survivor on
...
- G196 - NO-GO: B277 excludes its sole standard rank-102 survivor on
  \(Q^{14}\), together with the next five standard ranks.
- G197 - EXPLORATORY, active: classify cubic/quartic equality at rank
  103 on \(Q^{14}\), standard equality in dimensions 16--20, the
...
- NG229 - NO-GO: square equality \(6d+6\) cannot survive in dimensions
  14--20; G193 is reached.
- B274 - PROVED: the rank-65 standard equality on \(Q^{12}\) fails by
  the dimension-12 B257 branch audit, eliminating the sole residual
  branch of rank 95 on \(Q^{14}\); its standard floor is at least 96.
- NG230 - NO-GO: standard equality 95 cannot survive on \(Q^{14}\);
  G194 is reached.
- B275 - PROVED: the dimension-12 B258 audit excludes standard rank 66,
...
- NG232 - NO-GO: the standard band 97--101 cannot survive on \(Q^{14}\);
  G196 is reached.
- B277 - PROVED: B262 remains valid on \(Q^{12}\) and excludes ranks
  72--77; the lifted standard ranks 102--107 on \(Q^{14}\) fail, so its
  standard floor is at least 108.
- NG233 - NO-GO: the standard band 102--107 cannot survive on
  \(Q^{14}\); G197 is active with only cubic/quartic survivors there.
...
- NG101 - NO-GO: local $A_2$ versal nodalization cannot supply a clean
  multipart relation; a global topology-changing deformation is required.
...
- B077 - PROVED: proper pushdown of the pure constant object from the smooth
  semistable finite-stabilizer stack is pure, semisimple, and decomposes by
  strict support.
...
- G063 - EXPLORATORY, stronger same-chain gate: construct Saito's relative cycle,
  prove its boundary is the canonical full-support relation, and identify it
  with the B057 chain through both B022 quotients.
- B100 - PROVED: any two Saito relative lifts of the same local relation have
  the same primitive ambient image; their difference comes from nonprimitive
...
- G068 - EXPLORATORY, stronger optional mechanism: construct the collision pair,
...
- B105 - PROVED: the exact terminal discrepancy is
  $D_\zeta(c,\beta)=\langle\zeta,c-\Phi_{Y_0}(\beta)\rangle$, and Saito
  detection is equivalent to
  $D_\zeta(c,\beta)\ne\langle\zeta,c\rangle$.
...
- B001 - PROVED: universal HC is equivalent to universal middle-degree HC.
- G001 and G004 - EXPLORATORY: anchor access and universal injectively
  combined semiregular presentations remain open as a secondary route.
- B002-B004 - PROVED: Hilbert dominance and all-order semiregular propagation
  under their explicit hypotheses.
- B005-B006 and NG-006/NG-007 - PROVED/NO-GO: augmentation cannot repair an
  old kernel, and the low-degree projective product creates an uncontrolled
  obstruction summand.
