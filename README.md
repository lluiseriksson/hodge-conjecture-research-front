...
standard rational Hodge Conjecture
  ...
  <=> nonempty codimension-two local support                     [B012/G008]
      <= independent-node detector-span generation               [B016/G009]
      <= prescribed local-to-global Milnor defect                [B025/G010]
...
global input:
  ...
  independent nodes give the required local collision model      [B015, PROVED]
...
The active gate is **G010**, a concrete mechanism feeding G009 and G008.
B012 proves
...
primitive rational Hodge homology by Saito detector classes. G009 asks for
...
Hodge type and the nonzero pairing. B015 verifies the local geometry after a
suitable independent-node member is supplied, but does not construct that
member. A generic pencil misses the support, a net can meet it only after
...
terminal-equivalent support theorem, but remains open. G010 sharpens the next
obligation to an algebraic topology-changing degeneration that preserves a
global detector while producing a rational type-\((0,0)\) relation in the
global local-to-nearby-fiber kernel at one
