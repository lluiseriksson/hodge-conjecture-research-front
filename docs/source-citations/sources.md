...
- B224/G150 scope: Theorem 1.2 fixes the complementary degree
  \(s-a\), with \(s=\sum d_i-r-1\); it does not permit replacing that
  degree by an arbitrary \(m-2\). On a general smooth \(X\), B224 derives
  the corresponding adjunction twist directly. S070 constructs no
  canonical-bundle correction, weighted-Gale first-slack scheme,
  doubled/tripled jets, or rational detector.
...
## S081 - Even-quadric cohomology and ruling classes

Joseph Le Potier and Constantin Bănică, “Fibrés vectoriels topologiques
de rang élevé sur une hypersurface,” *Bulletin de la Société
Mathématique de France* **121** (1993), no. 2, 271–297,
doi:10.24033/bsmf.2209.
[NUMDAM article and PDF](https://www.numdam.org/articles/10.24033/bsmf.2209/).

- Audited Section 2.4, journal pp. 279–280 (PDF pp. 9–10), especially
  Proposition 2.5. For a smooth even-dimensional quadric \(X\) of
  dimension \(2m\), odd integral cohomology vanishes and even integral
  cohomology has rank one except in middle degree, where it is freely
  generated by \(a,b\).
- The paper identifies \(a,b\) as fundamental classes of maximal totally
  isotropic linear subspaces from the two families and proves
  \(a+b=h^m\) and \(h(a-b)=0\). Thus \(a-b\) is nonzero, primitive, and
  algebraic of middle Hodge type.
- B221 independently derives \(\operatorname{Pic}(X)=\mathbf Z[O_X(1)]\)
  from the recorded \(H^2\), the hypersurface exact sequence, standard
  projective-space cohomology, and the exponential sequence. It derives
  standard-quadric Gauss injectivity directly from the nondegenerate
  polar form; neither conclusion is attributed to Proposition 2.5.
- Scope guard: the source computes a special homogeneous variety. B221
