...
| mixed determinantal tangent | B171 computes G107's first tangent equation on W times ker E and excludes it under full gradient surjectivity with a uniform value matroid | NG135 shows even H_tau=0 and smooth excess need not satisfy it; G107 is optional, while G100 remains exact |
...
| downstairs IC descent | B039 excludes degree-one extra strict supports; B038 computes the polarized homological kernel model and B134 types the cohomological stalk as its dual | B040 computes the common Tate type; no class-specific nonvanishing follows |
...
| canonical detector grade | corrected B081/B121 separate constant ambient $E_\infty^{-2,1}$, relation/divisor $E_\infty^{-1,0}$, and point $E_\infty^{0,-1}$; B123 identifies the relation step with $\ker u_\Delta$ | NG058 closes arbitrary splitting; NG097 blocks ordinary-lift-to-relation promotion; NG099 proves every nonzero nearby class misses that step |
| proper support, Hodge type, and collision slice in the original hyperplane incidence map | B117 removes divisor support; B118 removes point support; conditional B119 controls relation type; B120-B122 make one transverse disk target ordinarily liftable | NG093-NG098 guard shifts, total type, full-plane invariance, omitted ambient grade, and raw-cocycle overconstraint; B123/NG099 close G070-G083 and redirect to G065's relative boundary |
...
| kernel-valued collision cocycle | B085 proves that a lift of a constant ambient class has defect in the combined B022 kernel and that invariant lifts are controlled by its class in $\operatorname{coker}(M_J-I)$ | NG062 blocks fixed-quotient-to-fixed-lift promotion; G050 is the explicit kernel matrix and cocycle calculation |
| finite versus unipotent collision monodromy | B086 kills every finite rational deck obstruction by Reynolds averaging; B087 identifies the residual unipotent obstruction with $[Nt]\in\operatorname{coker}N_J$ | NG063 blocks using finite averaging on the unipotent residue; G051 is the exact logarithmic matrix calculation |
| marked Hurwitz collision | B088 proves that pure Hurwitz motion fixes the geometric composite extension chain when the marked loop, reference fiber, and input class return exactly | NG064 blocks promoting unmarked Hurwitz/rank invariance to marked detector invariance; G052 must construct the marked topology-changing family |
| marked plane through a nodal target | B089 proves generic linear avoidance gives distinct local discriminant tangents while keeping a chosen smooth reference fiber fixed | NG065: the marked boundary loop need not inherit the global detector; B090/NG066 prove its fixed-class B057 coefficients actually vanish, so G054 must retain a nonlocal word |
| positive total nodal boundary | B090 proves a fixed class gives a relation vector orthogonal to itself, hence all rational coefficients and the ordered thimble extension vanish | G053 is NO-GO; this does not kill the abstract B009 local relation space, only the proposed total-boundary realization |
| pure Hurwitz localization | B091 proves that marked Hurwitz preservation plus identification with the positive local boundary would force a nonzero B058 detector chain to equal zero | NG067 closes path/basis mutation as the specialization mechanism; G055 must construct a genuine topology-changing correction |
| special lift versus local relation | B092 gives identical type-(0,0) nearby/special exact data with either zero or nonzero maps to a relation line | NG068: invariant-cycle liftability does not select the local coordinate; B093/G057 give the corrected canonical target and landing test |
| canonical dual-relation stalk | B093/B134 identify H^(-1) of the full-support perverse stalk with the dual relation group; B081 specifies its associated-grade access | NG107 forbids treating the class as a selected relation vector |
| affine special-lift torsor | B094 proves a detecting lift exists exactly when the base-lift value or ambiguity image under the scalar detector functional is nonzero | NG070 closes ambiguity-independence as a necessary condition; G058 is the exact geometric scalar computation |
| dual lift certificate | B095 rewrites B094 as a cokernel class of the dual special-to-nearby map, followed only in the zero class case by one descended evaluation | NG071: descent is not automatic and non-descent is favorable; G059 is the intrinsic two-case calculation |
| ambiguity boundary versus pairing square | B096 identifies the cokernel class with F composed with the preceding long-exact boundary and reduces the descended branch to equality with B058's nonzero pairing | NG072: liftability does not kill the boundary functional; G060 must compute the branch or square geometrically |
| constant ambient target square | B097 proves a quotient-compatible special/nearby morphism to constant primitive ambient homology kills ambiguity and preserves B058's nonzero pairing | NG073: raw proper pushforward does not establish B057 realization or B022-kernel descent; G061 is the exact morphism gate |
| fixed nearby quotient map | B098 proves both B022 kernels and the value c are already controlled on the nearby B057 class once it is realized | NG074: the nearby map need not agree automatically with B010's special Saito map; G062 is the exact topology-changing comparison |
...
| terminal scalar collision discrepancy | B105 proves the exact condition is D_zeta(c,beta) != <zeta,c> and identifies D_zeta with the paired ambient image of B104's coset when a compatible pair exists | NG081: nonzero relative cosets may die under ambient realization or pairing; G069 must construct the relation and prove the scalar inequality |
| auxiliary-detector cancellation | B106 proves c cancels from G069's inequality, leaving exactly B010, and restores B095's exhaustive cokernel/evaluation certificate on any valid collision domain | NG082: naming c in the discrepancy does not link the local relation to the global tube; B107/G070 determine the valid filtered domain |
| filtered collision certificate | B107 proves the relation functional is canonical only on the relevant perverse-filtration step S_0 and restricts B095's exact dual alternative to u_0=u|_(S_0) | NG083: an associated grade does not project the total stalk; B123 proves the required nonzero filtered lift cannot exist in the clean-nodal disk |
| filtered-lift obstruction | B108 identifies filtered liftability with omega_fil(t_psi)=[t_psi] in im(u)/u(S_0); B123 computes u(S_0)=0 | NG084: ordinary invariant-cycle surjectivity and Hodge strictness do not imply perverse strictness; NG099 closes the nonzero-target route |
| off-diagonal perverse extension | B109 identifies omega_fil with [s] in S/(S_0+ker u) and gives a dual witness; identical associated-graded maps can yield zero or nonzero obstruction | NG085 blocks rank-only inference; B123 supplies the clean-nodal geometric computation and redirects to G065 |
...
shortcut. B117 then performs the actual transverse calculation for the
original incidence pushdown. S052 makes $R^{2n}$ constant across a Lefschetz
...
NG110 records the failed sub-double-linear regime.

B138 resolves that former conic boundary and more. A minimal dependent
evaluation circuit is \(\mathrm{CB}(mn-c)\), so S057 places every circuit
...
B142 shows that this necessary regime is geometrically attainable in every
middle dimension. A general \((m,m)\) divisor in
...
B169 makes the full condition exact. Equation (2.2) of
Migliorini--Shende gives only the individual annihilator line arising from
each tracked critical point, so the local envelope is the union of the
nodal conormals. The nonzero generic ODP Euler jump puts each of those
conormals in the actual sheaf microsupport. Branchwise microlocal pullback
then vanishes exactly when the corresponding nodal divisor contains
...
B171 shows why this sufficient branch cannot replace the exact gate. Its
first tangent equation is mixed relation-Hessian vanishing on
\(W\times\ker E\), stronger than the B146 condition on
\(\ker E\times\ker E\). Full gradient surjectivity makes these equations
incompatible with a uniform rank-deficient value matroid. NG135 separates
the notions explicitly: \(\tau=(x,(1+y)x)\) has \(H_\tau=0\) and a smooth
height-one ideal, but \(\det d\tau=x\). G107 is therefore optional; the
active obligation returns to direct G100 syzygy lifting and the specified
pairing.
