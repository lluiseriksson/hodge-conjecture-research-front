...
| proper support and Hodge type in the original hyperplane incidence map | B117/S052 eliminate divisor pH^0 support; B118 reflects point pH^(-1) support into a constant high direct image; B119 identifies the forced remaining clean-nodal grade as type (0,0) without a type condition on the total lift | NG093-NG095 guard the two degree shifts and the total-lift overconstraint; G081 must construct the selected nonzero collision-invariant nearby class |
...
| filtered collision certificate | B107 proves the relation functional is canonical only on the relevant perverse-filtration step S_0 and restricts B095's exact dual alternative to u_0=u|_(S_0) | NG083: an associated grade does not project the total stalk; G070 must prove filtered liftability before computing either branch |
| filtered-lift obstruction | B108 identifies filtered liftability with omega_fil(t_psi)=[t_psi] in im(u)/u(S_0) | NG084: ordinary invariant-cycle surjectivity and Hodge strictness do not imply perverse strictness; G071 must kill the actual quotient class |
| off-diagonal perverse extension | B109 identifies omega_fil with [s] in S/(S_0+ker u) and gives a dual witness; identical associated-graded maps can yield zero or nonzero obstruction | NG085: E_infinity ranks and maps do not decide filtered liftability; G072 must compute the actual extension class |
...
even if the total rational lift is not. G081 now asks only for the selected
nonzero original nearby class and its full collision-monodromy invariance;
B084 supplies the rational lift.
