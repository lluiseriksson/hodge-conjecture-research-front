...
| minimal filtered boundary stratum | B133 identifies the two-branch ordinary target with ker(Q^2 -> span(delta_1,delta_2)); proportional cycles give the only nonzero two-node channel | NG106: a generic transverse double node has independent cycles and zero target; G088 still needs a class-directed relation and nonzero incidence coordinate |
...
| canonical relation stalk | B093 identifies the plane-net nodal relation group with H^(-1) of the full-support perverse stalk and B081 specifies its canonical associated-grade access | NG069 closes a direct total-derived-stalk projection; G057 is the corrected landing test |
